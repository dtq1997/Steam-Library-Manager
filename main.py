...
17.【弹窗定位】：所有弹窗（messagebox、simpledialog、filedialog）通过
   ui.py 中的 _patch_dialogs_topmost() 全局补丁自动查找正确的 parent 窗口。
   功能窗口（Toplevel）不强制 -topmost，以免遮挡用户其他工作。
18.【多来源合并模式】：当用户勾选了多个来源时，支持两种操作模式：
   a) 默认模式：每个来源分别独立导入/导出/更新为各自的收藏夹。
   b) 合并模式（取并集）：将所有勾选来源的游戏 ID 合并去重后，
      作为一个整体来源进行导入/导出/更新。合并后的名称默认为各来源名
      以" + "连接，用户可在确认步骤中修改。

================================================================================
【更新日志】
================================================================================
2026-02-10  v2.3.2 — 公司搜索显示 Steam 游戏数：
                    - 搜索公司后批量查询 involved_companies，结合本地缓存统计
                      每个公司关联的 Steam 游戏数，搜索结果按游戏数降序排列
                    - core.py 新增 count_igdb_company_steam_games() 批量统计方法
2026-02-10  v2.3.1 — 公司超链接 + 批量更新防溢出 + 禁止列拖拽：
                    - 开发商/发行商搜索结果新增 🔗 超链接列，点击跳转 IGDB 公司页面
                    - 合并模式的名称超过 60 字符时自动截断，防止弹窗过大
                    - 批量更新映射行的来源名称超过 50 字符时截断显示
                    - 所有 IGDB Treeview 列分隔线禁止拖拽（屏蔽 separator 事件）
2026-02-10  v2.3 — 公司标签页重做 + 全选支持 + 合并模式：
                    - 开发商/发行商标签页改用 Treeview 实现，与其他 IGDB 标签页视觉一致
                    - 「全选当前页」「取消全选当前页」按钮现已支持公司搜索结果
                    - 新增「🔗 合并所有勾选来源」选项：勾选后将多个来源取并集，
                      作为一个整体来源进行导入/导出/更新
                    - 导言区新增规则 18【多来源合并模式】
2026-02-10  v2.2.2 — 修复 IGDB 公司搜索无结果的问题：
                    - IGDB API 的 search 关键字对 companies 端点无效（返回空数组）
                    - 改用 where name ~ *"query"* 模糊匹配语法，搜索正常返回结果
2026-02-10  v2.2.1 — 「从推荐来源获取」体验优化 + IGDB 链接修复：
                    - 创建/更新收藏夹后窗口不再自动关闭，可连续操作
                    - 修复关键词（keywords）超链接地址格式错误（→ /categories/）
                    - 修复视角（player_perspectives）超链接地址格式错误（→ /player_perspectives/）
                    - 修复游戏模式（game_modes）超链接地址格式错误（→ /game_modes/）
2026-02-10  v2.2 — IGDB 界面大幅优化 + 取消窗口强制置顶：
                    - 所有功能窗口不再强制 -topmost，用户可自由切换窗口
                    - IGDB 分类列表改用 Treeview 实现，关键词等大列表不再卡顿
                    - 支持 macOS 触控板原生滚动（与主界面一致）
                    - 打开「从推荐来源获取」时自动加载分类列表，移除多余按钮
                    - 每个分类项右侧新增 🔗 链接，可直接在浏览器中查看 IGDB 页面
                    - IGDB API 查询新增 slug 字段，用于构建准确的 IGDB 链接
2026-02-10  v2.1.1 — 弹窗置顶机制改进：
                    - 改用「查找当前焦点 Toplevel 作为 parent」策略替代旧的
                      隐形锚点窗口方案，彻底解决 macOS 上 messagebox 被
                      Toplevel 子窗口遮挡的问题
                    - 同步覆盖 filedialog（文件选择对话框），补全遗漏
2026-02-10  v2.1 — IGDB 多维度分类 + 公司搜索 + 布局优化：
                    - 新增 6 个分类维度标签页：类型、主题、关键词、模式、视角、系列
                    - 新增「🏢 开发商/发行商」标签页：按名称搜索 IGDB 公司，
                      按需查询该公司关联的 Steam 游戏列表
                    - 每个标签页内置搜索框，支持快速过滤分类项
                    - 列表按关联游戏数降序排列，每项显示游戏数量
                    - 全选/取消仅作用于当前标签页（搜索后可只全选匹配项）
                    - 「从推荐来源获取」窗口改为左右两栏布局：
                      左栏 Steam250 + 鉴赏家，右栏 IGDB，底部按钮不再溢出
                    - 缓存格式升级为多维度结构（旧缓存需重新下载）
                    - 缓存中保存 game_to_steam 映射，加速公司搜索
